...
Igual, el caso de las 100 computadoras no sería tan perfecto como parece. Aunque cada máquina tendría su propio procesador y no competirían por núcleos, seguiría existiendo un tiempo "no paralelizable": repartir la tarea entre todas, esperar a que cada una termine, y al final juntar los resultados. Ese tiempo no desaparece por tener más máquinas, así que igual habría un límite de mejora, solo que más alto.

Además, en la vida real hay que sumarle el tiempo que toma comunicar las máquinas entre sí (enviar la tarea, recibir la respuesta de cada una), lo cual para una tarea tan pequeña como revisar una lista negra probablemente terminaría siendo más lento que hacerlo todo en una sola máquina con varios hilos. Y aunque no habría problema de núcleos compartidos, sí aparecerían otros dolores de cabeza propios de trabajar con varias máquinas a la vez, como fallos de red o que una máquina se caiga a mitad de proceso.
